...
class LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        c = torch.zeros(batch_size, hidden_size).to(x.device)
        out = torch.zeros(batch_size, seq_len, hidden_size).to(x.device)
        for ind in range(seq_len):
            x_i = x[:, ind, :]
            x_i = torch.cat([x_i, h], dim=1)

            i = F.sigmoid(self.Wi(x_i))
            f = F.sigmoid(self.Wf(x_i))
            o = F.sigmoid(self.Wo(x_i))
            c_hat = F.tanh(self.Wc(x_i))
...
net = Net(hidden_size=64, num_classes=2)
metric = Accuracy()
print(net)
